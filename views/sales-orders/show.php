...
<div class="toolbar">
    <h1 style="margin:0;">SO-<?= str_pad((string) $salesOrder->id, 5, '0', STR_PAD_LEFT) ?></h1>
    <span class="badge <?= $statusBadge ?>"><?= htmlspecialchars($salesOrder->status->label(), ENT_QUOTES) ?></span>
</div>
...
<?php if ($canSubmit || $canApproveOrReject || $canCancel): ?>
    <div class="card">
        <?php if ($canSubmit): ?>
            <form method="post" action="/sales-orders/<?= (int) $salesOrder->id ?>/submit" style="display:inline;">
                <button type="submit">Ajukan untuk Approval</button>
            </form>
        <?php endif; ?>
        <?php if ($canApproveOrReject): ?>
            <form method="post" action="/sales-orders/<?= (int) $salesOrder->id ?>/approve" style="display:inline;">
                <button type="submit">Setujui</button>
            </form>
            <form method="post" action="/sales-orders/<?= (int) $salesOrder->id ?>/reject" style="display:inline;">
                <button type="submit" class="btn-danger">Tolak</button>
            </form>
        <?php endif; ?>
        <?php if ($canCancel): ?>
            <form method="post" action="/sales-orders/<?= (int) $salesOrder->id ?>/cancel" style="display:inline;">
